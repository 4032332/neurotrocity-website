...
| **B6** | **Raster for the character, code for the geometry.** Speed lines, shockwave, impact frame, phone frames and the pacing bar are SVG/CSS | The aesthetic is flat tones and hard ink edges, which is SVG's native idiom. Coded geometry animates, stays crisp at any size, weighs nothing against §2.5, and lets the pacing bar be driven by real data (B8) |
...
| **B20** | **The free-test heading is Rob's joke; the offer is still stated plainly beneath it** | Rob, 24 Sep: *"the first hit is always free... that's how you end up addicted for life"*, replacing "Your first test is free." as the heading. That line moves into the body, verbatim, so a reader who misses the joke still reads the offer, and the page keeps a plain, sourced statement of what is free (D20). **Trade-off accepted knowingly (Rob, 24 Sep):** the final review found no s5M(8) engagement, but noted the joke sits in tension with the studio's own `RULES[1]` ("Honest by default. No manipulative nudges, no engineered regret."), since the app sells an auto-renewing subscription after the free test. Rob chose to keep it. The line stays off App Store surfaces: Apple's age-rating questionnaire asks about drug references |
| **B21** | **Ship without the flame and closing skulls; they appear when generated** | Rob, 24 Sep. Higgsfield's daily generation limit refused them three times. Each section renders its image only if the file exists (`src/content/beeptest-art.ts`) and goes single-column without it, so Task 12's art appears on the next build with no code change. A test fails on any broken image on the page |
